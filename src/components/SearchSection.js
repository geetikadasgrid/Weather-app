...
const SearchSection = ({ getWeatherApi, searchRef }) => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    const API_URL = `https://api.weatherapi.com/v1/forecast.json?key=${process.env.REACT_APP_API_KEY}&q=${searchInput}&days=5&aqi=yes`;
  ...
  const handleInputLocation = () => {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const API_URL = `https://api.weatherapi.com/v1/forecast.json?key=${process.env.REACT_APP_API_KEY}&q=${latitude},${longitude}&days=5&aqi=yes`;
        getWeatherApi(API_URL);
      },
      () => {
